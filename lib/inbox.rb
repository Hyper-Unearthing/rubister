...
class Inbox
  ...
  def next_pending(processing_timeout_seconds: 300)
    reclaim_stale_processing(processing_timeout_seconds: processing_timeout_seconds)
  ...
  def mark_failed(id, error:, max_attempts: 3)
    ...
    next_state = message.attempt_count >= max_attempts ? 'failed' : 'pending'

    Message.where(id: id).update_all(
      ["state = ?, last_error = ?, processing_started_at = NULL", next_state, error]
    )
  end
